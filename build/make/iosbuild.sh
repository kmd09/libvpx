...
build_framework() {
  ...
  if [ "$ENABLE_SHARED" = "yes" ]; then
    ...
    cp -p "${SCRIPT_DIR}/ios-module.modulemap" \
...
selfpath=`dirname $0`
VERSION=`"$selfpath"/version.sh --bare "$selfpath/../.." | sed -E 's/^v([0-9]+\.[0-9]+\.[0-9]+)(.*)$/\1/'`
if [ "$ENABLE_SHARED" = "yes" ]; then
  IOS_VERSION_OPTIONS="--enable-shared"
else
  IOS_VERSION_OPTIONS=""
fi
IOS_VERSION_MIN=`"$selfpath/ios-version.sh" $IOS_VERSION_OPTIONS`
